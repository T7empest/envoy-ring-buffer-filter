  ...
  namespace // helper functions
  {
    ...
    std::vector<Pool> parsePoolsFromStruct(const ProtobufWkt::Struct& s)
    {
      ...
      for (const auto& v : it->second.list_value().values())
      {
        ...
        // name
        std::string name;
        if (auto nit = ps.find("name"); nit != ps.end() && nit->second.kind_case() == ProtobufWkt::Value::kStringValue)
        {
        ...
        if (auto sit = ps.find("slots"); sit != ps.end() && sit->second.kind_case() == ProtobufWkt::Value::kNumberValue)
        {
        ...
        if (auto mit = ps.find("match"); mit != ps.end() && mit->second.kind_case() == ProtobufWkt::Value::kStructValue)
        {
          const auto& ms = mit->second.struct_value().fields();
          if (auto pit = ms.find("path_prefixes"); pit != ms.end() && pit->second.kind_case() == ProtobufWkt::Value::kListValue)
          {
